...
const Messages = ({ url }) => {
  ...
  const messageService = useMemo(() => {
    ...
    return {
      ...
      updateMessageStatus: async (id, statusData, signal) => {
        ...
        const response = await axios.patch(urlPatch, statusData, { signal });
        return response.data;
      },
      deleteMessage: async (id, signal) => {
        const urlDel = joinUrl(full, `${id}`);
        const response = await axios.delete(urlDel, { signal });
        return response.data;
      }
    };
  }, [url]);
  ...
  }, [messageService]); // re-run if messageService (hence url) changes
  ...
  const handleDelete = async (id) => {
    // Consider replacing with a bootstrap modal confirm for better UX
