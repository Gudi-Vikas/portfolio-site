...
const Projects = ({ url }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const resp = await axios.post(`${url}/api/projects/add`, formData, { headers: { Authorization: `Bearer ${localStorage.getItem('admin_token')||''}` } });
  ...
  const projectRemove = async (id) => {
    ...
    if (!ok) return;
    try {
      const resp = await axios.delete(`${url}/api/projects/project/${encodeURIComponent(id)}`, { headers: { Authorization: `Bearer ${localStorage.getItem('admin_token')||''}` } });
      if (resp.data.success) {
